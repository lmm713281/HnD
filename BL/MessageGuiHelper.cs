...
namespace SD.HnD.BL
{
	...
	public static class MessageGuiHelper
	{
		...
		public static int GetTotalNumberOfAttachmentsToApprove(List<int> accessableForums, List<int> forumsWithApprovalRight, List<int> forumsWithThreadsFromOthers, int userID)
		{
			...
			using(var adapter = new DataAccessAdapter())
			{
				...
				var q = qf.Create()
						  ...
						  .Where(CreateAttachmentFilter(accessableForums, forumsWithApprovalRight, forumsWithThreadsFromOthers, userID));
		...
		public static MessageEntity GetMessage(int messageID)
		{
			...
			using(var adapter = new DataAccessAdapter())
			{
				var message = adapter.FetchNewEntity<MessageEntity>(new RelationPredicateBucket(MessageFields.MessageID.Equal(messageID)));
		...
		public static DataView GetAttachmentsAsDataView(int messageID)
		{
			...
			using(var adapter = new DataAccessAdapter())
			{
				var attachments = adapter.FetchAsDataTable(q);
		...
		public static AttachmentEntity GetAttachment(int attachmentID)
		{
			using(var adapter = new DataAccessAdapter())
			{
				var toReturn = adapter.FetchNewEntity<AttachmentEntity>(new RelationPredicateBucket(AttachmentFields.AttachmentID==attachmentID));
