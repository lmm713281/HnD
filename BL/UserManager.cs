...
namespace SD.HnD.BL
{
	...
	public static class UserManager
	{
		...
		/// <param name="newBanFlagValue">the new value for the ban flag</param>
...
#warning UPDATE FOR #4
		...
		/// <returns>true if succeeded, false otherwise</returns>
		public static bool UpdateUserProfile(int userID, DateTime? dateOfBirth, string emailAddress, bool emailAddressIsPublic, string iconURL, string location, string occupation, 
											 string password, string signature, string website, int userTitleID, ParserData parserData, bool autoSubscribeThreads, short defaultMessagesPerPage)
		{
			...
			user.SignatureAsHTML = !string.IsNullOrEmpty(signature) ? TextParser.TransformSignatureUBBStringToHTML(signature, parserData) : string.Empty;
...
#warning UPDATE FOR #4
			...
			newUser.SignatureAsHTML = !string.IsNullOrEmpty(signature) ? TextParser.TransformSignatureUBBStringToHTML(signature, parserData) : string.Empty;
			...
			var roleUser = new RoleUserEntity
						   {
							   ...
							   User = newUser			// ties this new entity to newUser
						   };
